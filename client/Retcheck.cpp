...
uintptr_t Amp::Retcheck::Unprotect(uintptr_t addr)
{
    std::uint8_t* tAddr = (std::uint8_t*)addr;
    ...
    */
    ...
    DWORD funcSz = tAddr - (std::uint8_t*)addr;

    /* Allocate memory for the new function */
    void* nFunc = VirtualAlloc(NULL, funcSz, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (nFunc == NULL)
        return addr;
    ...
    std::uint8_t* pos = (std::uint8_t*)nFunc;
    bool valid = false;
    do
    {
